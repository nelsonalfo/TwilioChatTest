...
import com.example.miguelsoler.twiliochattest.Conexion.SessionManager;
  ...
  private void populateChannels() {
    ...
    channelManager.populateChannels(new LoadChannelListener() {
      ...
      public void onChannelsFinishedLoading(List<Channel> channels) {
        Log.e("MAINACTIVITY", "onChannelsFinishedLoading " + channels.get(0).getUniqueName());
  ...
  private void initializeClient() {
    chatClientManager.connectClient(new TaskCompletionListener<Void, String>() {
      ...
      public void onSuccess(Void aVoid) {
        Log.e("MAINCHATACTIVITY", "initializeClient() access " + String.valueOf(aVoid));
        populateChannels();
      }

      @Override
      public void onError(String errorMessage) {
        Log.e("MAINCHATACTIVITY", "initializeClient() onError " + errorMessage);
//        stopActivityIndicator();
      ...
      public void run() {
        AlertDialogHandler.displayCancellableAlertWithHandler(message, context,
            new DialogInterface.OnClickListener() {
              @Override
              public void onClick(DialogInterface dialog, int which) {
                SessionManager.getInstance().cleanChannel();
                finish();
              }
            });
      }
